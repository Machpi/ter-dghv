...
def encrypt_with_client(b, p, client_path="./client"):
  b_str = str(b)
  p_str = str(p)
  try:
...
def decrypt_with_client(ciphertext, secret_key, client_path="./client"):
...
def encrypt_image(input_filename, p, client_path="./client"):
    encrypted_values = []
    with open(input_filename, 'r') as f:
        for line in f:
            ...
            for char in line:
                ...
                bit = int(char)
                encrypted_bit = encrypt_with_client(bit, p, client_path)
...
def decrypt_image(encrypted_filename, p, client_path="./client"):
    try:
        ...
        for i, enc_val in enumerate(encrypted_values):
            try:
                decrypted_bit = decrypt_with_client(enc_val, p, client_path)
        ...
        output_filename = encrypted_filename.replace('.enc', '.dec')
        with open(output_filename, 'w') as f:
